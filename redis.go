...
type Client interface {
	...
	// Redis HDEL command.
	Hdel(key string, hashkey string) (result int64, err Error)

	// Redis HGETALL command.
	Hgetall(key string) (result map[string][]byte, err Error)

	// Redis HKEYS command.
	Hkeys(key string) (result []string, err Error)

	// Redis HLEN command
	Hlen(arg0 string) (result int64, err Error)
...
type AsyncClient interface {
	...
	// Redis HGET command.
	Hget(key string, key2 string) (result FutureBytes, err Error)

	// Redis HSET command.
	Hset(key string, key2 string, arg1 []byte) (status FutureBool, err Error)

	// Redis HGETALL command
	Hgetall(arg0 string) (result FutureKVs, err Error)

	// Redis HKEYS command.
	Hkeys(key string) (result FutureKeys, err Error)

	// Redis HLEN command.
	Hlen(key string) (result FutureInt64, err Error)

...
func (c *syncClient) Hgetall(arg0 string) (result map[string][]byte, err Error) {
	arg0bytes := []byte(arg0)

	var resp Response
	resp, err = c.conn.ServiceRequest(&HGETALL, [][]byte{arg0bytes})
	if err == nil {
		result = parseKVs(resp.GetMultiBulkData())
	}
	return result, err
...
// Redis HKEYS command.
func (c *syncClient) Hkeys(arg0 string) (result []string, err Error) {
	arg0bytes := []byte(arg0)

	var resp Response
	resp, err = c.conn.ServiceRequest(&HKEYS, [][]byte{arg0bytes})
	if err == nil {
		//		result = strings.Split(bytes.NewBuffer(resp.GetBulkData()).String(), " ", 0);
		res := resp.GetMultiBulkData()
		result = make([]string, len(res))
...
// Redis HLEN command
func (c *syncClient) Hlen(arg0 string) (result int64, err Error) {
	arg0bytes := []byte(arg0)

	var resp Response
	resp, err = c.conn.ServiceRequest(&HLEN, [][]byte{arg0bytes})
	if err == nil {
		result = resp.GetNumberValue()
	}
	return result, err
...
// Redis HDEL command.
func (c *syncClient) Hdel(arg0, arg1 string) (result int64, err Error) {
	arg0bytes := []byte(arg0)
	arg1bytes := []byte(arg1)

	var resp Response
	resp, err = c.conn.ServiceRequest(&HDEL, [][]byte{arg0bytes, arg1bytes})
	if err == nil {
		result = resp.GetNumberValue()
	}
	return result, err

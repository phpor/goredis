...
func (c *asyncClient) Keys(arg0 string) (result FutureKeys, err Error) {
	...
	if err == nil {
		result = newFutureKeys(resp.future.(FutureBytesArray))
	}
	return result, err
}
...
func (c *asyncClient) Hgetall(arg0 string) (result FutureKVs, err Error) {
	arg0bytes := []byte(arg0)

	var resp *PendingResponse
	resp, err = c.conn.QueueRequest(&HGETALL, [][]byte{arg0bytes})
	if err == nil {
		result = newFutureKVs(resp.future.(FutureBytesArray))
	}
	return result, err

}
// Redis HLEN command.
func (c *asyncClient) Hlen(arg0 string) (result FutureInt64, err Error) {
	arg0bytes := []byte(arg0)

	var resp *PendingResponse
	resp, err = c.conn.QueueRequest(&HLEN, [][]byte{arg0bytes})
	if err == nil {
		result = resp.future.(FutureInt64)
	}
	return result, err
...
// Redis KEYS command.
func (c *asyncClient) Hkeys(arg0 string) (result FutureKeys, err Error) {
	arg0bytes := []byte(arg0)

	var resp *PendingResponse
	resp, err = c.conn.QueueRequest(&HKEYS, [][]byte{arg0bytes})
	if err == nil {
		result = newFutureKeys(resp.future.(FutureBytesArray))
	}
	return result, err
